...
@click.option('--width', 
              default=80, 
              help='Ancho en caracteres de la salida')
@click.option('--height',
              type=int,
              help='Alto en caracteres (opcional)')
...
@click.option('--remove-bg',
              is_flag=True,
              help='Intentar remover el fondo')
...
@click.option('--verbose', '-v',
              ...
              help='Modo verboso')
@click.pass_context
def main(ctx, mode, file_path, width, height, charset, no_color, remove_bg, output, verbose):
    ...
    if ctx.invoked_subcommand is None:
        ...
        try:
            ...
            if not file_path:
                ...
                if not file_path:
                    console.print("[red]❌ No se encontró ningún archivo válido en el directorio actual[/red]")
                    show_usage_help()
                    sys.exit(1)
...
def process_image_cli(file_path: str, converter: ASCIIConverter, remove_bg: bool, output: Optional[str]):
    ...
    with Progress(
        ...
        try:
            ...
            ascii_result = processor.process_image(file_path, remove_bg=remove_bg, enhance=True)
...
def process_gif_cli(file_path: str, converter: ASCIIConverter, output: Optional[str]):
    ...
    if output:
        ...
        console.print(f"[green]✅ {len(created_files)} frames exportados a: {output_dir}[/green]")
        ...
        console.print("[yellow]🎬 Reproduciendo GIF ASCII (Ctrl+C para detener)[/yellow]")
...
@click.option('--output-dir', '-o', default='./ascii_frames', help='Directorio de salida')
...
def scan(directory):
    ...
    with console.status(f"[bold green]🔍 Escaneando {dir_path}..."):
        images = list(dir_path.glob('*.jpg')) + list(dir_path.glob('*.jpeg')) + \
                list(dir_path.glob('*.png')) + list(dir_path.glob('*.bmp')) + \
                list(dir_path.glob('*.tiff')) + list(dir_path.glob('*.webp'))
        
        gifs = list(dir_path.glob('*.gif'))
    
    if not images and not gifs:
        console.print(f"[yellow]⚠️  No se encontraron archivos compatibles en {dir_path}[/yellow]")
        return
