...
class FileValidator:
    ...
    def validate_file(cls, file_path: str, file_type: str = 'image') -> Path:
        ...
        if file_size > cls.MAX_FILE_SIZE:
            raise ValidationError(f"Archivo muy grande: {file_size/1024/1024:.1f}MB > {cls.MAX_FILE_SIZE/1024/1024}MB")
        
        # Verificar extensión
        file_ext = path.suffix.lower()
        supported_exts = cls.SUPPORTED_FORMATS.get(file_type, [])
        if file_ext not in supported_exts:
            raise ValidationError(f"Formato no soportado: {file_ext}. Soportados: {supported_exts}")
    ...
    @classmethod
    def find_first_file(cls, directory: str = ".", file_type: str = 'image') -> Optional[Path]:
